                  ...
                  if(eval(i).test(fullpath)){
                     ...
                     for(var j=0;j<that.getArr[i].attr.length;j++){
                      ...
                      res.render=function(tpl,data){
                          var tplUrl=path.join(that.tplDir,tpl);

                          fs.readFile(tplUrl,function(error,data1){
                              if(error){
                                  res.end("this is template is no find");
                              }else{
                                  var result=ejs(data1.toString(),data);
                                  res.end(result);
                              }
                          })
